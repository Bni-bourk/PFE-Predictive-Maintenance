...
void loop() {
  ...
  for (int i = 0; i < nodeCount; i++) {
    if (nodes[i].lastSeen > 0 && (now - nodes[i].lastSeen > 15000)) {
      if (nodes[i].id == "M1") mb.Hreg(4, 0); // Drop M1 Comms to Error (0)
      if (nodes[i].id == "M2") mb.Hreg(14, 0); // Drop M2 Comms to Error (0)
    }
  }
    ...
    if (radio.readData(rxdata) == RADIOLIB_ERR_NONE) {
      ...
      if (!deserializeJson(doc, rxdata)) {
        ...
        if (idx >= 0) {
          ...
          // --- MODBUS STATIC ROUTING (The Fix) ---
          int stateInt = (nodes[idx].status == "NORMAL") ? 1 : 0;
          int16_t snrInt = (int16_t)(nodes[idx].snr * 10.0);
          uint16_t vibInt = (uint16_t)(nodes[idx].value * 100.0);

          if (id == "M1") {
            mb.Hreg(0, nodes[idx].battery);
            mb.Hreg(1, snrInt);
            mb.Hreg(2, vibInt);
            mb.Hreg(3, stateInt);
            mb.Hreg(4, 1); // Comm OK
            ...
            mb.Hreg(10, nodes[idx].battery);
            mb.Hreg(11, snrInt);
            mb.Hreg(12, vibInt);
            mb.Hreg(13, stateInt);
            mb.Hreg(14, 1); // Comm OK
          }
        }
      }
    }
